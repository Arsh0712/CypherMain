...
class AntiVC(commands.Cog):
    ...
    async def on_voice_state_update(self, member, before, after):
        if after.channel and await self.is_banned(member.guild.id, member, after.channel.id):
            ...
            await member.send(f"<:MekoExclamation:1302243537924132875> {member.mention} you are not allowed to join voice channels in **{member.guild.name}**.")
    ...
    async def antivc_add(self, ctx, target: discord.Member | discord.Role, channel: discord.VoiceChannel = None):
        ...
        await ctx.send(embed=discord.Embed(
            description=f"<a:H_TICK:1302972336039006229> {ctx.author.mention}: added {'role' if is_role else 'user'} {target.mention} to anti-vc ban list {'for ' + channel.mention if channel else 'for all VCs'}.",
            color=0x2f3136
        ))
    ...
    async def antivc_remove(self, ctx, target: discord.Member | discord.Role, channel: discord.VoiceChannel = None):
        ...
        if cursor.rowcount == 0:
            await ctx.send(embed=discord.Embed(
                description=f"<:MekoExclamation:1302243537924132875> {ctx.author.mention}: no ban found for {target.mention}.",
                color=0x2f3136
                ))
            return
        
        await ctx.send(embed=discord.Embed(
            description=f"<a:H_TICK:1302972336039006229> {ctx.author.mention}: removed {'role' if is_role else 'user'} {target.mention} from anti-vc ban list {'for ' + channel.mention if channel else 'for all VCs'}.",
            color=0x2f3136
            ))
    ...
    async def antivc_config(self, ctx):
        ...
        if not results:
            return await ctx.send(embed=discord.Embed(
                description=f"<:MekoExclamation:1302243537924132875> {ctx.author.mention}: no entries found.",
                color=0x2f3136
            ))
