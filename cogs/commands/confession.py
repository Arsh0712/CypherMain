...
class Confessions(commands.Cog):
    ...
    async def enable(self, ctx, channel: discord.TextChannel):
        ...
        await ctx.send(embed=discord.Embed(description=f"<a:H_TICK:1302972336039006229> {ctx.author.mention}: Confession channel set to {channel.mention}."))
    ...
    async def disable(self, ctx):
        ...
        await ctx.send(embed=discord.Embed(description=f"<a:H_TICK:1302972336039006229> {ctx.author.mention}: Confessions have been disabled for this server."))
    ...
    async def confess(self, interaction: discord.Interaction):
        if self.is_user_muted(interaction.guild.id, interaction.user.id):
            await interaction.response.send_message(embed=discord.Embed(description=f"<:MekoExclamation:1302243537924132875> {interaction.user.mention}: You are muted from submitting confessions in this server.",color=0xFEE75C), ephemeral=True)
            return
        ...
        if not result or not result[0]:
            await interaction.response.send_message(embed=discord.Embed(description=f"<:MekoExclamation:1302243537924132875> {interaction.user.mention}: Confessions are disabled in this server.",color=0xFEE75C), ephemeral=True)
            return
    ...
    async def mute(self, ctx, user: discord.User):
        ...
        self.db.commit()
        await ctx.send(embed=discord.Embed(description=f"<a:H_TICK:1302972336039006229> {ctx.author.mention}: {user.mention} has been muted from using confessions."))
    ...
    async def unmute(self, ctx, user: discord.User):
        ...
        self.db.commit()
        await ctx.send(embed=discord.Embed(description=f"<a:H_TICK:1302972336039006229> {ctx.author.mention}: {user.mention} has been unmuted and can now use confessions again."))
