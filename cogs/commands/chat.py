        ...
        if result:
            ...
            if log_channel:
                await log_channel.send(embed=discord.Embed(
                    ...
                    description=f"<:MekoMod:1302242489415110676> Moderator : {ctx.author.mention}\n"
                                f"<:MekoMember:1302243182804860938> Target : {target.mention} ({'Role' if is_role else 'User'})\n"
                                f"<:MekoRuby:1303665457965826089> Scope : {'All Channels' if channel is None else channel.mention}",
                    color=discord.Color.red()
                ))
