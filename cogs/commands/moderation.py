...
class PrefixChangeView(discord.ui.View):
    ...
    async def reset_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(
            embed=discord.Embed(
                description=f"<a:H_TICK:1302972336039006229> Prefix has been reset to default: `{self.default_prefix}`",
    ...
    async def set_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await self.ctx.send(
                embed=discord.Embed(
                    description=f"<a:H_TICK:1302972336039006229> Prefix has been changed to `{new_prefix}`",
                    color=discord.Color.green()
                )
            )
...
async def do_removal(ctx, limit, predicate, *, before=None, after=None):
    ...
    deleted = len(deleted)
    embed = discord.Embed(
        description=f'<a:H_TICK:1302972336039006229> Sucessfully purged {deleted} messages.',
    ...
    if len(to_send) > 2000:
        await ctx.send(f"<a:H_TICK:1302972336039006229> Successfully removed {deleted} messages.", delete_after=10)
    else:
...
class Moderation(commands.Cog):
  ...
  async def unlockall(self, ctx):
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
          button = Button(label="Yes",
                    ...
                    emoji="<a:H_TICK:1302972336039006229>")
          button1 = Button(label="No",
                     style=discord.ButtonStyle.red,
                     emoji="<a:MekoCross:1302972393572143125>")
                  ...
                  if interaction.guild.me.guild_permissions.administrator:
                    ...
                    description=f"Unlocking all channels in {ctx.guild.name}.")
                      ...
                      await interaction.channel.send(
                              content=f"<a:H_TICK:1302972336039006229> Successfully unlocked {a} channel's .")   
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                           "<:MekoExclamation:1302243537924132875> Insufficient Privileges: This command cannot be executed due to the absence of required permissions.\n Please ensure the necessary authorization is granted to proceed.",
                              embed=None,
                                  view=None)
              else:
                embed = discord.Embed()
                embed.description = f"<:MekoExclamation:1302243537924132875> I'm sorry, **{interaction.user.name}**, you cannot access this button.\n<:MekoArrowRight:1302273350743097395> Please use the bot command first then you can access this button"
        ...
        description=
        """<:MekoExclamation:1302243537924132875> You need Administrator permissions.\n<:MekoExclamation:1302243537924132875> Your role must be higher than mine.""",
        color=self.color)
                    ...
                    description=f"Locking all channels in {ctx.guild.name}.")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<a:H_TICK:1302972336039006229> Successfully locked {a} channel's.")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                    ...
                    description=f"Hiding all channels in {ctx.guild.name}.")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<a:H_TICK:1302972336039006229> Successfully hidden {a} channel's.")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                    ...
                    description=f"Unhiding all channels in {ctx.guild.name}.")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<a:H_TICK:1302972336039006229> Successfully unhidden {a} channel's")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
  ...
  async def _unhide(self, ctx, channel: discord.TextChannel = None):
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            ...
            try:
                ...
                embed = discord.Embed(
                    color=self.color,
                    description=f"<a:H_TICK:1302972336039006229> Successfully unhidden {channel.mention}."
                )
  ...
  async def _hide(self, ctx, channel: discord.TextChannel = None):
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            ...
            try:
                ...
                embed = discord.Embed(
                    color=self.color,
                    description=f"<a:H_TICK:1302972336039006229> Successfully hidden {channel.mention}."
                )
                await ctx.send(embed=embed)
            except Exception as e:
                print(e)
  ...
  async def clone(self, ctx: commands.Context, channel: discord.TextChannel):
    ...
    hacker = discord.Embed(
      color=self.color,
      description=
      f"<a:H_TICK:1302972336039006229> {channel.name} has been successfully cloned"
    )
  ...
  async def changenickname(self, ctx: commands.Context, member: discord.Member,*, name: str = None):
        ...
        try:
            ...
            error = discord.Embed(
                color=self.color,
                description="<a:MekoCross:1302972393572143125> I don't have permission to manage this user's nickname!"
        )
            await ctx.send(embed=error)
        except Exception as e:
            error = discord.Embed(
                color=self.color,
                description=f"<a:MekoCross:1302972393572143125> An error occurred while trying to change the nickname: {str(e)}"
                )
            await ctx.send(embed=error)
  ...
  async def _unlock(self,
    ...
    if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
      ...
      try:
          ...
          embed = discord.Embed(
            color=self.color,
            description=f"<a:H_TICK:1302972336039006229> Successfully unlocked {channel.mention}."
            )
          await ctx.send(embed=embed)
      except Exception as e:
        print(e)
  ...
  async def _lock(self,
    ...
    if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
      ...
      try:
          ...
          embed = discord.Embed(
            color=self.color,
            description=f"<a:H_TICK:1302972336039006229> Successfully locked {channel.mention}."
            )
          await ctx.send(embed=embed)
      except Exception as e:
        print(e)
  ...
  async def addsticker(self, ctx: commands.Context, *, name=None):
        ...
        try:
            ...
            s = await ctx.guild.create_sticker(name=name, description= f"Sticker created by {str(self.bot.user)}", emoji="<:MekoEmoji:1303972522546561047>", file=file)
            await ctx.reply(f"<a:H_TICK:1302972336039006229> Sucessfully created sticker with name `{name}`", stickers=[s])
        except:
  ...
  async def role_humans(self, ctx, *, role: discord.Role):
    if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
        ...
        async def button_callback(interaction: discord.Interaction):
            ...
            if interaction.user == ctx.author:
                if interaction.guild.me.guild_permissions.manage_roles:
                    ...
                    await interaction.channel.send(
                        content=f"<a:H_TICK:1302972336039006229> Successfully assigned {role.mention} to {count} human(s).")
                else:
                    await interaction.response.edit_message(
                        content="<:MekoExclamation:1302243537924132875> I am missing the required permissions. Please grant the necessary permissions and try again.",
                        embed=None,
                        view=None)
            else:
                embed = discord.Embed()
        ...
        if len(members_without_role) == 0:
            return await ctx.reply(embed=discord.Embed(description=f"<:MekoExclamation:1302243537924132875> All humans already have the {role.mention} role.", color=self.color))
        else:
            embed = discord.Embed(
                color=self.color,
        ...
        denied = discord.Embed(
            description="<a:MekoCross:1302972393572143125> Your role should be above my top role.",
            color=0x000000)
        denied.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=denied, mention_author=False)
  ...
  async def role_bots(self, ctx, *, role: discord.Role):
    if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
        ...
        async def button_callback(interaction: discord.Interaction):
            ...
            if interaction.user == ctx.author:
                if interaction.guild.me.guild_permissions.manage_roles:
                    ...
                    await interaction.channel.send(
                        content=f"<a:H_TICK:1302972336039006229> Successfully added {role.mention} to {count} bot(s).")
                else:
                    await interaction.response.edit_message(
                        content="<:MekoExclamation:1302243537924132875> I am missing the required permission. Please grant the necessary permissions and try again.",
                        embed=None,
                        view=None)
            else:
                embed = discord.Embed()
        ...
        if len(bots_without_role) == 0:
            return await ctx.reply(embed=discord.Embed(description=f"<:MekoExclamation:1302243537924132875> All bots already have the {role.mention} role.", color=self.color))
        else:
            embed = discord.Embed(
                color=self.color,
  ...
  async def roleicon(self, ctx: commands.Context, role: discord.Role, *, icon: Union[discord.Emoji, discord.PartialEmoji, str]=None):
        if role.position >= ctx.guild.me.top_role.position:
            em = discord.Embed(description=f"<:MekoExclamation:1302243537924132875> {role.mention} role is higher than my role, move it to the top!", color=self.color)
        if ctx.author.top_role.position <= role.position:
            em = discord.Embed(description=f"<:MekoExclamation:1302243537924132875> {role.mention} has the same or higher position from your top role!", color=self.color)
        ...
        if icon is None:
            ...
            if c:
                try:
                    ...
                    em = discord.Embed(description=f"<a:H_TICK:1302972336039006229> Successfully changed icon of {role.mention}", color=self.color)
                except:
                    return await ctx.reply("Failed to change the icon of the role")
            else:
                await role.edit(display_icon=None)
                em = discord.Embed(description=f"<a:H_TICK:1302972336039006229> Successfully removed icon from {role.mention}", color=self.color)
        ...
        if isinstance(icon, discord.Emoji) or isinstance(icon, discord.PartialEmoji):
            ...
            try:
              ...
              return await ctx.reply("Failed to change the icon of the role")
            await role.edit(display_icon=img)
            em = discord.Embed(description=f"<a:H_TICK:1302972336039006229> Successfully changed the icon for {role.mention} to {icon}", color=self.color)
            return await ctx.reply(embed=em, mention_author=False)
        else:
            ...
            await role.edit(display_icon=img)
            em = discord.Embed(description=f"<a:H_TICK:1302972336039006229> Successfully changed the icon for {role.mention}", color=self.color)
            return await ctx.reply(embed=em, mention_author=False)
  ...
  async def _reactions(self, ctx, search=100):
        ...
        await ctx.success(f"<a:H_TICK:1302972336039006229> Successfully removed {total_reactions} reactions.")
