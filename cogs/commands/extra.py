...
class VoteView(discord.ui.View):
    def __init__(self, support_url):
        ...
        self.add_item(discord.ui.Button(
            emoji=f"<:dbl:1310528459130277949>",
            label="Vote on DBL",
            url="https://discord.ly/cypher-2733",
            style=discord.ButtonStyle.link
        ))
        self.add_item(discord.ui.Button(
            emoji=f"<:MekoTopgg:1302243523441066034>",
...
class InviteView(discord.ui.View):
    def __init__(self, bot_id):
        ...
        # Invite button
        self.add_item(discord.ui.Button(
            emoji=f"<:MekoAdd:1302243175460769803>",
            label=f"Invite Me!",
...
class SupportView(discord.ui.View):
    def __init__(self, support_url):
        super().__init__()
        self.add_item(discord.ui.Button(
            emoji=f"<:Support:1309767390174904361>",
            label="Support Me!",
            url=support_url,
            style=discord.ButtonStyle.link
        ))
...
tick = "<a:H_TICK:1302972336039006229>"
cross = "<a:MekoCross:1302972393572143125>"
...
class Extra(commands.Cog):
  ...
  def __init__(self, bot):
    self.bot = bot
    self.color = 0x2f3136
    self.support_url = "https://discord.gg/m4Hbeftf8B"
  ...
  async def vote(self, ctx: commands.Context):
    ...
    embed.set_author(name=f"Consider Voting me",
                     icon_url=self.bot.user.avatar.url)
    embed.description = f"<:upvote:1310529013646888982> {ctx.author.mention}: [DBL](https://discord.ly/https://discord.ly/cypher-2733)/[Top.gg](https://top.gg/bot/1191399940014997584/vote) vote from the button below!"
  ...
  async def botinfo(self, ctx: commands.Context):
        ...
        embed = discord.Embed(colour=0x2b2d31, description=f"**<:1181899559853621419:1264778211544530974> Uptime: {uptime}**\n**<:ExoticUser:1264778296479191091> Users: {total_users}**\n**<:MekoChannel:1302243558748717057> Channels: {channel}**\n**<:ExoticGuild:1264778405749194915> Guilds: {total_guilds}**\n**<:MekoApplication:1303967760258174976> Shards: {ctx.guild.shard_id+1}/{len(self.bot.shards.items())}**\n**<:ExoticPing:1264791506586439715> Latency: {latency:.3f}ms**\n**<:ExoticCPU:1264797431229579294> Total Load: {cpu_percent}%**\n**<:MekoUtility:1302243552092356608> Storage: {using_storage_mb:.2f} MB / 2.00 GB ({disk_usage.percent}%)**\n<:ExoticRam:1264798433164923042> **Ram Info: {used_ram_mb:.2f} MB / 9.98 GB**\n**<:ExoticPython:1264800097544306740> Python: {python_version}**\n**<:ExoticCores:1264800568371839027> Cores: {cpu_total_cores}**\n**<:wl_dark:1264800901777064017> Music Wrapper: Lavalink.Py 5.9.0**\n**<:ExoticOS:1264801368091267113> Os: {os_name}**")
  ...
  async def serverinfo(self, ctx: commands.Context):
    ...
    embed.add_field(
      name="<:MekoServer:1302243161359253546> **__About Server__**",
      value=
      f"> Server Name :  {guild.name}\n> Server ID : {guild.id}\n> Server Owner : {guild.owner} (<@{guild.owner_id}>)\n> Created :  <t:{c_at}:F>\n> Members Count : {len(guild.members)}",
      inline=False)

    embed.add_field(
      name="<:MekoUtility:1302243552092356608> **__Server Settings__**",
      value=
    ...
    embed.add_field(name="<:MekoNewLines:1303665143002959902> **__Description__**",
                    value=f"""> {guild.description}""",
                    inline=False)
    if guild.features:
      ftrs = ("\n").join([f" {'> <a:H_TICK:1302972336039006229>'+' : '+feature.replace('_',' ').title()}" for feature in guild.features])

      embed.add_field(

        name="<:MekoRuby:1303665457965826089> **__Server Features__**",

        value=ftrs[:1024]
      )
      

    embed.add_field(name="<:MekoMember:1302243182804860938> **__Total Members__**",
                    value=f"""
...
> Bots : {len(list(filter(lambda m: m.bot, guild.members)))}
    ...
    #for vc1 in
    embed.add_field(name="<:MekoCategory:1302243555254730784> **__Channels__**",
                    value=f"""
...
> Threads : {len(guild.threads)}
    ...
    embed.add_field(name="<:Sticker:1303668007809056774> **__Emojis & Stickers Info__**",
                    value=f"""
...
> Total Emoji/Stickers : {total_emojis}
    ...
    embed.add_field(
      name="<:MekoBoost:1302242845457125398> **__Boost Status__**",
      value=
    ...
    embed.add_field(name=f"<:Meko_Role:1302243570467602515>**__Server Roles__**",
  ...
  async def _userinfo(self,
    ...
    if member in ctx.guild.members:
      if member == ctx.guild.owner:
        aklm = "<:MekoOwner:1302242752662081546> Server Owner"
      elif member.guild_permissions.administrator:
        aklm = "<:MekoAdmin:1303717979430588547> Server Admin"
      elif member.guild_permissions.ban_members or member.guild_permissions.kick_members:
        aklm = "<:icon_moderator:1303718081931247628> Server Moderator"
      else:
        aklm = "<:MekoUser:1302242484427948172> Server Member"
    ...
    embed.add_field(name="<:MekoMember:1302243182804860938> __About__",
                    value=f"""
> Default Name : {member}
> ID : {member.id}
> Global Name : {member.global_name}
> Bot? : {'<a:H_TICK:1302972336039006229>' if member.bot else '<a:MekoCross:1302972393572143125>'}
> Badges : {badges}
...
> Server Joined : {joinedat}
    ...
    if member in ctx.guild.members:
      ...
      embed.add_field(name="<:Meko_Role:1302243570467602515> __User Roles Info__",
                      value=f"""
...
> Hex Colour : {member.color if member.color else '000000'}
    ...
    if member in ctx.guild.members:
      embed.add_field(
        name="<:MekoRuby:1303665457965826089> __Extra__",
        value=
        ...
        inline=False)
    if member in ctx.guild.members:
      embed.add_field(name="<:MekoKey:1303715594280898562> __Key Permissions__",
                      value=", ".join([kp]),
                      inline=False)
    if member in ctx.guild.members:
      embed.add_field(name="<:MekoSearch:1302242746932789270> __Acknowledgement__",
                      value=f"{aklm}",
                      inline=False)
    if member in ctx.guild.members:
      embed.set_footer(text=f"Requested by {ctx.author}",
  ...
  async def roleinfo(self, ctx: commands.Context, *, role: discord.Role):
    ...
    description += (
        f"<:MekoRoleYellow:1303971384812371970>**Name :** {role.name}\n"
        f"<:MekoPing:1302242859537272852>**Mention :** {role.mention}\n"
        f"<:id:1303722827748343972>**ID :** {role.id}\n"
        f"<:MekoTimer:1303972088893145133>**Created at :** {role.created_at.strftime('%d/%m/%Y %H:%M')}\n"
        f"<:MekoEmoji:1303972522546561047>**Hex Color :** {str(role.color).upper()}\n"
        f"<:MekoType:1303972689282601011>**Mentionable :** {role.mentionable}\n"
        f"<:MekoRuby:1303665457965826089>**Hoisted :** {str(role.hoist)}\n"
        f"<:MekoMember:1302243182804860938>**Members :** {len(role.members)}"
    )
    ...
    if perms:
      description += f"\n\n<:MekoKey:1303715594280898562>**Allowed permissions :** {' '.join(perms)}"
...
##########
    ...
    if hacker in sup.members: 
      if dev in hacker.roles:
        bdg += "\n<:MekoDeveloper:1303313515716677662> Developer"
      if chief in hacker.roles:
        bdg += "\n<:MekoOwner:1302242752662081546> Owner"
      if visor in hacker.roles:
        bdg += "\n<:MekoEmoji:1303972522546561047> Founder"
        
      if sdent in hacker.roles:
        bdg += "\n<:MekoRoleYellow:1303971384812371970> Manager"
      if sup1 in hacker.roles:
        bdg += "\n<:MekoAdmin:1303717979430588547> Admin"

      if pillar in hacker.roles:
        bdg += "\n<:oxytech_pillar:1100288688195387405> Mod"   
      if premium in hacker.roles:
        bdg += "\n<:MekoPrefix:1303236683591974963> No Prefix"
      ...
      if blesser in hacker.roles:
        bdg += "\n<:NexusStarboard:1304026018906116136> Friend"
        
      if loved in hacker.roles:
        bdg += "\n<a:MekoLove:1303235997399646208> Cypher's Special"

      if pals in hacker.roles:
        bdg += "\n<:OxyTech_developers:1100282130795335720> *Pals*"
      if member in hacker.roles:
        bdg += "\n<:MekoUser:1302242484427948172> Bot User"      
      ...
      #embed2.add_field(name="User Badges",
                     #  value=f"{badges}",
                     #  inline=False)
      embed2.add_field(name="<:MekoReddot:1303229652466139177> List Of Badges", value=f"\u200b{bdg}", inline=False)
      embed2.add_field(name="\u200b\n<:MekoType:1303972689282601011> Commands Used", value=f"```{command_count}```", inline=False)
      embed2.add_field(
        name="",
        value=(
          f"\u200b\nIn Order To Receive Your Badges, You Must Be Present In Our Support Server. "
          f"To Join The Support Server Click [**Here**](https://discord.gg/hBr4KJTb8v)."
          ),
        inline=False)
      embed2.set_author(
