    ...
    async def add(self, ctx, role: discord.Role):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (ctx.guild.id,)) as cursor:
                ...
                if row:
                    embed = discord.Embed(
                                          description=f"<:MekoExclamation:1302243537924132875> VC role is already set in this guild with the role {ctx.guild.get_role(row[0]).mention}.\nPlease **remove** it to add another one.", color=0x000000)
                    await ctx.reply(embed=embed)
                    return
            await db.execute('INSERT INTO vcroles (guild_id, role_id) VALUES (?, ?)', (ctx.guild.id, role.id))
            await db.commit()
            embed = discord.Embed(
                                  description=f"<a:H_TICK:1302972336039006229> Successfully added VC role {role.mention} for this guild.", color=0x000000)
            await ctx.reply(embed=embed)
    ...
    async def remove(self, ctx, role: discord.Role):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ? AND role_id = ?', (ctx.guild.id, role.id)) as cursor:
                ...
                if not row:
                    embed = discord.Embed(
                                          description="<:MekoExclamation:1302243537924132875> Given role is not set in VC role.", color=0x000000)
                    await ctx.send(embed=embed)
                    return
            await db.execute('DELETE FROM vcroles WHERE guild_id = ? AND role_id = ?', (ctx.guild.id, role.id))
            await db.commit()
            embed = discord.Embed(
                                  description=f"<a:H_TICK:1302972336039006229> Successfully removed VC role {role.mention} for this guild.", color=0x000000)
            await ctx.send(embed=embed)
    ...
    async def config(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (ctx.guild.id,)) as cursor:
                ...
                if not row:
                    embed = discord.Embed(
                                          description="<:MekoExclamation:1302243537924132875> VC role is not set in this guild.", color=0x000000)
                    await ctx.send(embed=embed)
                    return
            ...
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (member.guild.id,)) as cursor:
                    ...
                    if after.channel and role not in member.roles:
                        await self.add_role_with_retry(member, role, reason="Member Joined VC | Cypher's Invcrole")
                    elif not after.channel and role in member.roles:
                        await self.remove_role_with_retry(member, role, reason="Member Left VC | Cypher's Invcrole")
...
async def setup(client):
    await client.add_cog(Invcrole(client))