...
class Ignore(commands.Cog):
  ...
  async def channel_add(self, ctx: commands.Context, channel: discord.TextChannel):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if count[0] >= 30:
        embed = discord.Embed(description="<:MekoExclamation:1302243537924132875> You can only add up to 30 channels to the ignore list.", color=self.color)
      ...
      if result:
        embed = discord.Embed(description=f"<a:MekoCross:1302972393572143125> {channel.mention} is already in the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
        await db.execute("INSERT INTO ignored_channels (guild_id, channel_id) VALUES (?, ?)", (ctx.guild.id, channel.id))
        await db.commit()
        embed = discord.Embed(description=f"<a:H_TICK:1302972336039006229> Successfully added {channel.mention} to the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def channel_remove(self, ctx: commands.Context, channel: discord.TextChannel):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        embed = discord.Embed(description=f"<a:MekoCross:1302972393572143125> {channel.mention} is not in the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed)
      else:
        await db.execute("DELETE FROM ignored_channels WHERE guild_id = ? AND channel_id = ?", (ctx.guild.id, channel.id))
        await db.commit()
        embed = discord.Embed(description=f"<a:H_TICK:1302972336039006229> Successfully removed {channel.mention} from the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def user_add(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if count[0] >= 30:
        embed = discord.Embed(description="<:MekoExclamation:1302243537924132875> You can only add up to 30 users to the ignore list.", color=self.color)
        await ctx.reply(embed=embed)
        return
      ...
      if result:
        embed = discord.Embed(description=f"<a:MekoCross:1302972393572143125> {user.mention} is already in the ignore users list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
        await db.execute("INSERT INTO ignored_users (guild_id, user_id) VALUES (?, ?)", (ctx.guild.id, user.id))
        await db.commit()
        embed = discord.Embed(description=f"<a:H_TICK:1302972336039006229> Successfully added {user.mention} to the ignore users list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def user_remove(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        embed = discord.Embed(description=f"<a:MekoCross:1302972393572143125> {user.mention} is not in the ignore users list.", color=self.color)
        await ctx.reply(embed=embed)
      else:
        await db.execute("DELETE FROM ignored_users WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, user.id))
        await db.commit()
        embed = discord.Embed(description=f"<a:H_TICK:1302972336039006229> Successfully removed {user.mention} from the ignore users list.", color=self.color)
        await ctx.send(embed=embed)
  ...
  async def bypass_add(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if count[0] >= 30:
        embed = discord.Embed(description="<:MekoExclamation:1302243537924132875> You can only add up to 30 users to the bypass list.", color=self.color)
        await ctx.reply(embed=embed)
        return
      ...
      if result:
        embed = discord.Embed(description=f"<a:MekoCross:1302972393572143125> {user.mention} is already in the bypass users list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
        await db.execute("INSERT INTO bypassed_users (guild_id, user_id) VALUES (?, ?)", (ctx.guild.id, user.id))
        await db.commit()
        embed = discord.Embed(description=f"<a:H_TICK:1302972336039006229> Successfully added {user.mention} to the bypass users list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def bypass_remove(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        embed = discord.Embed(description=f"<a:MekoCross:1302972393572143125> {user.mention} is not in the bypass users list.", color=self.color)
        await ctx.reply(embed=embed)
      else:
        await db.execute("DELETE FROM bypassed_users WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, user.id))
        await db.commit()
        embed = discord.Embed(description=f"<a:H_TICK:1302972336039006229> Successfully removed {user.mention} from the bypass users list.", color=self.color)
        await ctx.reply(embed=embed)
