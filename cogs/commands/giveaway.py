...
class Giveaway(commands.Cog):
    ...
    async def end_giveaway(self, giveaway):
        try:
            ...
            for reaction in message.reactions:
                if str(reaction.emoji) == "<:MekoGift:1302243150685016124>":
            ...
            await message.edit(content="<:MekoGift:1302243150685016124> **Giveaway Ended** <:MekoGift:1302243150685016124>", embed=embed)
            await message.reply(f"<a:MekoLove:1303235997399646208> Congratulations {winner_mentions}! You've won the giveaway!")
            await self.cursor.execute("DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?", (message.id, message.guild.id))
            await self.connection.commit()
    ...
    async def gstart(self, ctx, time, winners: int, *, prize: str):
        ...
        embed = discord.Embed(
            description=f"**Ends:** <t:{round(ends)}:R> (<t:{round(ends)}:f>)\n"
                       f"**React with <:MekoGift:1302243150685016124> to participate.**\n"
                       f"**Hosted By:** {ctx.author.mention}\n"
                       f"**Winners:** {winners}", 
            color=0x2f3136)
        embed.set_author(name=prize, icon_url=ctx.guild.icon.url)
        ends_utc = datetime.datetime.utcnow() + datetime.timedelta(seconds=converted)
        embed.timestamp = ends_utc
        embed.set_footer(text="Ends at", icon_url=ctx.bot.user.avatar.url)

        message = await ctx.send("<:MekoGift:1302243150685016124> **Giveaway Started** <:MekoGift:1302243150685016124>", embed=embed)
        try:
        ...
        await self.connection.commit()
        await message.add_reaction("<:MekoGift:1302243150685016124>")
    ...
    async def greroll(self, ctx, message_id: typing.Optional[int] = None):
        ...
        if not message.content.startswith("<:MekoGift:1302243150685016124> **Giveaway Ended**"):
        ...
        await ctx.send(f"<a:MekoLove:1303235997399646208> Congratulations <@{winner}>! You've won the rerolled giveaway!")
