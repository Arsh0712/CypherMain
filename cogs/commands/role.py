...
class Server(commands.Cog):
                        ...
                        if context.author == context.guild.owner or reqrole in context.author.roles:
                            if role:
                                if role not in member.roles:
                                    ...
                                    embed = discord.Embed(
                                        description=f"<a:H_TICK:1302972336039006229> **Successully Given** <@&{role.id}> To {member.mention}",
                                        color=0x000000)
                                else:
                                    await self.remove_role2(role=role_id, member=member)
                                    embed = discord.Embed(
                                        description=f"<a:H_TICK:1302972336039006229> **Sucessfully Removed** <@&{role.id}> From {member.mention}",
                                        color=0x000000)
                                await context.reply(embed=embed)
                            else:
                                embed = discord.Embed(
                                    description=f"<a:MekoCross:1302972393572143125> {role_type.capitalize()} role is not set up in {context.guild.name}",
                                    color=0x000000)
                                await context.reply(embed=embed)
                        else:
                            embed = discord.Embed(
                                description=f"<:MekoExclamation:1302243537924132875> You need {reqrole.mention} to run this command.",
                                color=0x000000)
                            await context.reply(embed=embed)
                    else:
                        embed = discord.Embed(
                            description=f"<:MekoExclamation:1302243537924132875> Required role is not set up in {context.guild.name}",
                            color=0x000000)
                        await context.reply(embed=embed)
                else:
                    embed = discord.Embed(
                        description=f"<a:MekoCross:1302972393572143125> Roles configuration is not set up in {context.guild.name}",
                        color=0x000000)
                    await context.reply(embed=embed)
    ...
    async def staff(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            embed = discord.Embed(
                description=f"<a:H_TICK:1302972336039006229> Successully Added {role.mention} to `Staff` Role.",
                color=0x000000)
            await context.reply(embed=embed)
        else:
            embed = discord.Embed(
                                  description="<:MekoExclamation:1302243537924132875> Your role should be above my top role.",
                                  color=0x000000)
            await context.reply(embed=embed)
    ...
    async def girl(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            embed = discord.Embed(
                description=f"<a:H_TICK:1302972336039006229> Successfully Added {role.mention} to `Girl` Role.",
                color=0x000000)
            await context.reply(embed=embed)
        else:
            embed = discord.Embed(
    ...
    async def vip(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            embed = discord.Embed(
                description=f"<a:H_TICK:1302972336039006229> Successfully Added {role.mention} to `VIP` Role.",
                color=0x000000)
            await context.reply(embed=embed)
        else:
            embed = discord.Embed(
    ...
    async def guest(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            embed = discord.Embed(
                description=f"<a:H_TICK:1302972336039006229> Successfully Added {role.mention} to `Guest` Role. ",
                color=0x000000)
            await context.reply(embed=embed)
        else:
            embed = discord.Embed(
    ...
    async def friend(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            embed = discord.Embed(
                description=f"<a:H_TICK:1302972336039006229> Successfully Added {role.mention} to `Friend` Role. ",
                color=0x000000)
            await context.reply(embed=embed)
        else:
            embed = discord.Embed(
    ...
    async def req_role(self, context: Context, role: discord.Role) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            embed = discord.Embed(
                color=0x000000,
                description=f"<a:H_TICK:1302972336039006229> Successfully Added {role.mention} for Required role to run custom role commands in {context.guild.name}"
            )
            await context.reply(embed=embed)
        else:
            embed = discord.Embed(
    ...
    async def config(self, context: Context) -> None:
        ...
        if role_data:
            ...
            embed = discord.Embed(
                description="<:MekoExclamation:1302243537924132875> There is no custom role configuration found in this Guild.",
                color=0x000000
            )
    ...
    async def create(self, context: Context, name: str, role: discord.Role) -> None:
        async with aiosqlite.connect(DATABASE_PATH) as db:
            async with db.execute("SELECT COUNT(*) FROM custom_roles WHERE guild_id = ?", (context.guild.id,)) as cursor:
                ...
                if count[0] >= 56:
                    embed = discord.Embed(
                        description="<:MekoExclamation:1302243537924132875> You have reached the maximum limit of 56 custom role commands for this guild.",
                        color=0x000000
                    )
                    await context.reply(embed=embed)
                    return
            ...
            async with db.execute("SELECT name FROM custom_roles WHERE guild_id = ?", (context.guild.id,)) as cursor:
                ...
                if any(name == row[0] for row in existing_role):
                    embed = discord.Embed(
                        description=f"<:MekoExclamation:1302243537924132875> A custom role command with the name `{name}` already exists in this guild. Remove it before creating a new one.",
                        color=0x000000
                    )
                    await context.reply(embed=embed)
                    return
        ...
        embed = discord.Embed(
            description=f"<a:H_TICK:1302972336039006229> Successfully created Custom role command `{name}`  to assign the role {role.mention}.",
            color=0x000000
        )
        await context.reply(embed=embed)
    ...
    async def delete(self, context: Context, name: str) -> None:
        ...
        if not existing_role:
            embed = discord.Embed(
                description=f"<:MekoExclamation:1302243537924132875> There is no custom role command with the name `{name}` was found in this guild.",
                color=0x000000
            )
            await context.reply(embed=embed)
            return
        ...
        embed = discord.Embed(
            description=f"<a:H_TICK:1302972336039006229> Custom role command `{name}` has been deleted.",
            color=0x000000
        )
        await context.reply(embed=embed)
    ...
    async def list(self, context: Context) -> None:
        ...
        if not custom_roles:
            embed = discord.Embed(
                description="<:MekoExclamation:1302243537924132875> No custom roles have been created for this server.",
                color=0x000000
            )
            await context.reply(embed=embed)
            return
    ...
    async def reset(self, context: Context) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            description=f"Deleted All Custom Role commands <a:H_TICK:1302972336039006229>\n\n**Removed Roles:**\n" + "\n".join(removed_roles) if removed_roles else "No roles were previously set.",
            color=0x000000
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if result:
            ...
            if reqrole is None:
                await message.channel.send("<:MekoExclamation:1302243537924132875> The required role is not set up in this server. Please set it up using `setup reqrole`.")
                return

            
            if reqrole not in message.author.roles:
                await message.channel.send(embed=discord.Embed(description=f"<:MekoExclamation:1302243537924132875> You need the {reqrole.mention} role to use this command.", color=0x000000))
                return
            ...
            try:
                if role in member.roles:
                    ...
                    await message.channel.send(embed=discord.Embed(
                        description=f"<a:H_TICK:1302972336039006229>** Successfully removed** the role {role.mention} from {member.mention}.",
                        color=0x000000
                    ))
                else:
                    await self.add_role(role_id=role_id, member=member)
                    await message.channel.send(embed=discord.Embed(
                        description=f"<a:H_TICK:1302972336039006229> **Successfully added** the role {role.mention} to {member.mention}.",
                        color=0x000000
                    ))
