...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await self.user.send(f"<a:H_TICK:1302972336039006229> You have been Unbanned from **{self.author.guild.name}** by **{self.author}**. Reason: {reason or 'Not Provided'}")
            dm_status = "Yes"
...
class Ban(commands.Cog):
    ...
    async def ban(self, ctx, user: discord.User, *, reason=None):
        ...
        try:
            await user.send(f"<:MekoExclamation:1302243537924132875> You have been banned from **{ctx.guild.name}** by **{ctx.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"<:MekoMod:1302242489415110676> Moderator : [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id}) ({ctx.author.mention})\n<:MekoMember:1302243182804860938> Target : [{user.display_name}](https://discord.com/users/{user.id}) ({user.mention})\n<:icon_mail:1332957321424404541> Dm : {dm_status}\n<:MekoRuby:1303665457965826089> Reason : {reasonn}", color=self.color)
