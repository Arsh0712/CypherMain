...
def progress_bar(current: int, total: int, size: int = 20) -> str:
    ...
    completed_marker = '<:bar_full_pink:1335197152045039657>'
    remaining_marker = '<a:bar_empty:1335197150367191043>'
    current_position_marker = '<:cypher:1318607934871306300>'
    ...
    def build_embed(self, page: int) -> discord.Embed:
        ...
        embed = discord.Embed(
            description=(
                ...
                f"<a:whiteheart:1342876782273237002> `1` [{current_track.title}]({current_track.uri}) ・ {current_track.author} ・ `{current_duration_str}`"
            ),
            color=self.color
...
class MusicControlView(discord.ui.View):
    ...
    async def pause_resume(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not interaction.user.voice:
            await interaction.response.send_message("<:error_emoji:1255171928986226730> You are not in a voice channel.", ephemeral=True)
            return
        if interaction.user.voice.channel != interaction.guild.voice_client.channel:
            await interaction.response.send_message("<:stolen_emoji:1255171928986226730> You are not in the same voice channel.", ephemeral=True)
            return
    ...
    async def shuffle(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not interaction.guild.voice_client:
            await interaction.response.send_message("<:stolen_emoji:1255171928986226730> I am not connected to any voice channel.", ephemeral=True)
            return
        if self.player.paused:
            await interaction.response.send_message("<:stolen_emoji:1255171928986226730> I am currently paused, please use `.resume`.", ephemeral=True)
            return
...
class Music(commands.Cog):
    ...
    async def toggle(self, ctx):
        ...
        embed1.description = "**.toggle**\n\n**Subcommands**\n`1.` **.toggle source** <:ecl_white_arrow:1260632276107137087> Sets default source of music ( for you )\n`2.` **.toggle volume** <:ecl_white_arrow:1260632276107137087> Sets default volume of music ( for you )"
    ...
    async def toggle_volume(self, ctx: commands.Context, vol: int):
        ...
        if not (0 <= vol <= 200):
            embed = discord.Embed(description="<:stolen_emoji:1255171779031203864> Please provide valid volume levels between 0 and 200.", colour=0x2e2e2e)
        ...
        embed = discord.Embed(description=f"<:Discord_voice_white:1269291307218632848> | Successfully set the volume to {new_volume}%.", colour=0x2e2e2e)
    ...
    async def toggle_source(self, ctx):
        ...
        # Define the buttons
        youtube_button = Button(emoji="<:YouTube:1275459057649586280>", label="YouTube", style=discord.ButtonStyle.secondary, custom_id="source_youtube")
        jio_button = Button(emoji="<:Spotify:1319569561686904914> ", label="Spotify", style=discord.ButtonStyle.secondary, custom_id="source_spotify")
        soundcloud_button = Button(emoji="<:soundcloud:1275459746106966109>", label="SoundCloud", style=discord.ButtonStyle.secondary, custom_id="source_soundcloud")
    ...
    async def play(self, ctx: commands.Context, *, query: str):
        ...
        if results['loadType'] == 'PLAYLIST_LOADED':
            ...
            embed = discord.Embed(
                description="<a:pinkkitty:1342755371537793125> **Added tracks to the queue**",
                color=0x2e2e2e
            )
    ...
    async def nowplaying(self, ctx):
        ...
        embed6.description=f"<:WickTick:1324739295319298130> **Now Playing - {player.current.title}**\n\n<:stolen_emoji:1255171898359287838> **Duration :** {duration_str}\n<:stolen_emoji:1255171898359287838> **Author :** {author}\n<:stolen_emoji:1255171898359287838> **Requested by :** {requester.display_name}\n<:emoji_20:1325813769498136576> **{bar}**"
    ...
    async def optimize_vc(self, ctx):
      ...
      embed = discord.Embed(description=f"<a:MekoLoading:1302242510411796480> **Adjusting parameters for a richer and fuller sound !**")
      ...
      try:
        ...
        embed = discord.Embed(
          description=f"<a:H_TICK:1302972336039006229> Set voice channel bitrate to **{bitrate // 1000}** kbps\n"
          f"<a:H_TICK:1302972336039006229> Set voice channel region to **India**\n"
          f"<a:H_TICK:1302972336039006229> Set vol to **80%** to reduce distortions\n"
          f"<a:H_TICK:1302972336039006229> Optimized audio for best listening experience.",
