...
class AntinukeGroups(commands.Cog):
    ...
    async def antinuke_enable(self, ctx: commands.Context):
        ...
        if ctx.author.id == ctx.guild.owner_id:
           if data == "on":
            ...
            embed = discord.Embed(description=f"<:MekoExclamation:1302243537924132875> {ctx.author.mention}: You must be the guild owner to use this command", color=0xFFFFFF)
            await ctx.send(embed=embed)
    ...
    async def antinuke_show(self, ctx: commands.Context):
        ...
        if user_id != str(ctx.guild.owner_id):
                embed = discord.Embed(description=f"<:MekoExclamation:1302243537924132875> {ctx.author.mention}: You must be the guild owner to use this command", color=0xFEE75C)
                await ctx.send(embed=embed)
                return
        ...
        if data == "off":
            description = (f"**{ctx.guild.name} Antinuke Configurations**\n\n"
                          ...
                          "Current Status: <:disabled:1264303333015945216>\n\n"
            ...
            module_status = "\n".join(f"**{name}:** {'<:enable:1264303466461794385>' if status == 'on' else '<:disabled:1264303333015945216>'}" for name, status in modules.items())
            ...
            description = (f"**{ctx.guild.name} Antinuke Configurations**\n\n"
                          ...
                          f"**Auto Recovery:** <:enable:1264303466461794385>\n"
    ...
    async def recover(self, ctx: commands.Context):
        ...
        if ctx.author.id != ctx.guild.owner_id and str(ctx.author.id) not in owner:
            em1 = discord.Embed(description="<:Icon_No:1154259733142835240> | This command can only be used by the guild owner or extra owners.", color=embed_color)
            return await ctx.send(embed=em1, delete_after=120)
        if ded == "off":
            em4 = discord.Embed(description="<:Icon_No:1154259733142835240> | Antinuke is not enabled for this guild.", color=embed_color)
    ...
    async def module(self, ctx: commands.Context):
        ...
        if ded == "off":
            ...
            embed.set_footer(text=f"Requested By {ctx.author.display_name}",
                            icon_url=ctx.author.display_avatar.url)
            b2 = Button(label="Need Help? Join Support Server", style=discord.ButtonStyle.url, url="https://discord.gg/XBbHRYg4ZW")
            view = View()
            view.add_item(b2)
    ...
    async def owner_add(self, ctx, user: discord.User):
        ...
        if ctx.author.id == ctx.guild.owner_id:
            if len(own) >= max_owners:
                embed = discord.Embed(
                    description=f"<:Icon_No:1154259733142835240> | This server has reached the maximum number of extra owners ({max_owners}).\n- Remove one to add another.\n- Add up-to 5 extraowners by having premium",
                    color=clr
                )
                await ctx.send(embed=embed)
            else:
                if str(user.id) in own:
                    embed = discord.Embed(
                        description=f"<:Icon_No:1154259733142835240> | {user.mention} is already an extra owner for this server.",
                        color=clr
                    )
                    await ctx.send(embed=embed)
                else:
                    ...
                    embed = discord.Embed(
                        description=f"<:tick_icons:1154295677149519894> | Successfully added **{user.mention}** to extra owner.",
                        color=clr
                    )
                    await ctx.send(embed=embed)
        else:
            await ctx.send("<:Icon_No:1154259733142835240> | You must be the guild owner to add someone as an extra owner.")
    ...
    async def owner_remove(self, ctx, user: discord.User):
      ...
      if ctx.author.id == ctx.guild.owner_id:
        if str(user.id) in own:
          ...
          updateConfig(ctx.guild.id, data)
          embed = discord.Embed(description=f"<:tick_icons:1154295677149519894> | Successfully removed **{user.mention}** from extraowner.", color=clr)
          await ctx.send(embed=embed)
        else:
          embed = discord.Embed(description=f"<:Icon_No:1154259733142835240> | {user.mention} Is not an extraowner for this server .",
                                  color=clr
            )
          await ctx.send(embed=embed)
      else:
        await ctx.send("<:Icon_No:1154259733142835240> | You must be the guild owner to remove someone from extra owners.")
    ...
    async def owner_show(self, ctx):
        ...
        if ctx.author.id == ctx.guild.owner_id :
            if not own:
                await ctx.send(f"<:Icon_No:1154259733142835240> | There aren't any extraowner(s) to show. You can add them by `{prefix}extraowner add <user>`")
            else:
            ...
            await ctx.send("<:Icon_No:1154259733142835240> | You must be the guild owner to view the extra owners.")
    ...
    async def whitelist_add(self, ctx, user: discord.User):
        ...
        if ctx.author == ctx.guild.owner or str(ctx.author.id) in ownerr: 
            if len(wled) >= max_whitelisted:
                embed = discord.Embed(
                    description=f"<:Icon_No:1154259733142835240> | This server has reached the maximum number of extra owners ({max_whitelisted}).\n- Remove one to add another.\n- Add up-to 5 extraowners by having premium",
                    color=clr
                )
                await ctx.send(embed=embed)
            elif ded == "off":
                em4 = discord.Embed(
                    description="<:Icon_No:1154259733142835240> | Antinuke is not enabled for this guild.", 
                    color=clr
                )
                ...
                if str(user.id) in wled:
                    embed = discord.Embed(
                        description=f"<:Icon_No:1154259733142835240> | {user.mention} is already whitelisted.", 
                        color=clr
                    )
                    await ctx.send(embed=embed)
                else:
                    ...
                    updateConfig(ctx.guild.id, data)
                    embed = discord.Embed(
                        description=f"<:tick_icons:1154295677149519894> | Successfully whitelisted **{user.mention}**.", 
                        color=clr
                    )
                    await ctx.send(embed=embed)
        else:
            await ctx.send("<:Icon_No:1154259733142835240> | You must be the guild owner or an extra owner to whitelist someone.")
    ...
    async def whitelist_remove(self, ctx, user: discord.User):
      ...
      if ctx.author == owner or str(ctx.author.id) in ownerr: 
        if ded == "off":
            em4 = discord.Embed(description="<:Icon_No:1154259733142835240> | Antinuke is not enabled for this guild.", color=clr)
            return await ctx.send(embed=em4, delete_after=120) 
        if str(user.id) in wled:
          wled.remove(str(user.id))
          updateConfig(ctx.guild.id, data)
          embed = discord.Embed(description=f"<:tick_icons:1154295677149519894> | Successfully unwhitelisted **{user.mention}**.", color=clr)
          await ctx.send(embed=embed)
        else:
          embed = discord.Embed(description=f"<:Icon_No:1154259733142835240> | {user.mention} Is not whitelisted.", color=clr)
          await ctx.send(embed=embed)
      else:
        await ctx.send("<:Icon_No:1154259733142835240> | You must be the guild owner to remove someone from whitelist.", mention_author=False)
    ...
    async def whitelist_show(self, ctx):
        ...
        if ctx.author == owner or str(ctx.author.id) in ownerr: 
            if ded == "off":
                ...
                return await ctx.send(embed=em4, delete_after=120) 
            if len(wled) == 0:
                await ctx.send(f"<:Icon_No:1154259733142835240> | There aren't any whitelisted user(s). You can add them by `{prefix}antinuke wl add <user>`")
            else:
           ...
           await ctx.send("<:Icon_No:1154259733142835240> | You must be the guild owner to view the whitelisted users.", mention_author=False)
    ...
    async def wl_reset(self, ctx):
        ...
        if ctx.author == ownerr or str(ctx.author.id) in owner:
            if not d3["whitelisted"]:
                em5 = discord.Embed(description="<:Icon_No:1154259733142835240> | The whitelist is already empty. There is nothing to reset.", color=embed_color)
                return await ctx.send(embed=em5, delete_after=120)
            d3["whitelisted"] = []
            updateConfig(ctx.guild.id, d3)
            em = discord.Embed(description="<:tick_icons:1154295677149519894> | Successfully reset the whitelist for this server.", color=embed_color)
            await ctx.send(embed=em)
        else:
            await ctx.send("<:Icon_No:1154259733142835240> | You must be the guild owner or an extra owner to use this command.")
