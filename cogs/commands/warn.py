...
class Warn(commands.Cog):
    ...
    async def warn_user(self, user: discord.Member, total_warns: int):
        ...
        if timeout_duration:
            try:
                ...
                return f"\n<:MekoTimer:1302243147547541514> Timed Out : `{timeout_duration // 60}m.`"
            except discord.Forbidden:
                return "\n<:olympusError:1234567890> I do not have permission to timeout this user."
            except discord.HTTPException:
                return "\n<:olympusError:1234567890> Failed to timeout the user due to an error."
        return ""
    ...
    async def warn(self, ctx, user: discord.Member, *, reason=None):
        ...
        try:
            ...
            embed = discord.Embed(description=f"<:MekoMod:1302242489415110676> Moderator : [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id}) ({ctx.author.mention})\n"
                                              f"<:MekoMember:1302243182804860938> Target : [{user.display_name}](https://discord.com/users/{user.id}) ({user.mention})\n"
                                              f"<:icon_mail:1332957321424404541> Dm : {dm_status}\n"
                                              f"<:MekoExclamation:1302243537924132875> Warns : {total_warns}{punishment_msg}\n"
                                              f"<:MekoRuby:1303665457965826089> Reason : `{reason_to_send}`",
                                              color=self.color)
    ...
    async def clearwarns(self, ctx, user: discord.Member):
        try:
            ...
            embed = discord.Embed(description=f"<a:H_TICK:1302972336039006229> All warnings have been cleared for **{user}** in this guild.", color=self.color)
