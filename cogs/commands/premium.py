...
class Premium(commands.Cog):
    ...
    async def generate(self, ctx, duration: str, guild_count: int):
        ...
        embed.description =f"Hey, {ctx.author.display_name} I've Generated a code for you.\n\n<:MekoTimer:1302243147547541514> **Expires at :** {duration}\n<:MekoRuby:1303665457965826089> **Valid for :** {guild_count} guild(s)\n<:MekoArrowRight:1302273350743097395> **Redeem Code :**\n```\n{code}```"
    ...
    async def activate(self, ctx, code: str):
                ...
                if existing_premium:
                    ...
                    if existing_expires_at > discord.utils.utcnow():
                        ...
                        embed.description = (
                            ...
                            f"<:MekoTimer:1302243147547541514> **Expires at :** <t:{expires_timestamp}:F>"
                        )
                        await ctx.send(embed=embed)
                        return
        ...
        embed.description = (
            ...
            f"<:MekoRuby:1303665457965826089> **Activated at :** <t:{activated_timestamp}:F>\n"
            f"<:MekoTimer:1302243147547541514> **Ends at :** <t:{expires_timestamp}:F>"
        )
    ...
    async def stats(self, ctx):
        ...
        embed.description = (
            ...
            f"<:MekoRuby:1303665457965826089> **Premium activated :** <t:{activated_timestamp}:F>\n"
            f"<:MekoTimer:1302243147547541514> **Premium ends :** <t:{expires_timestamp}:F>\n\n"
            f"<:MekoArrowRight:1302273350743097395> **You have added premium to {guild_count} guild(s):** \n"
            f"{', '.join(guild_names)}"
        )
    ...
    async def revoke(self, ctx, user: discord.User):
        async with aiosqlite.connect('database/premium_codes.db') as db:
            async with db.execute('SELECT 1 FROM premium_users WHERE user_id = ?', (user.id,)) as cursor:
                ...
                if row is None:
                    embed = discord.Embed(
                        title="<:MekoExclamation:1302243537924132875> No Premium",
    ...
    async def add_np(self, user, duration):
        ...
        embed = discord.Embed(
            title="<:MekoGift:1302243150685016124> You got No Prefix!",
    ...
    async def remove_np(self, user):
        ...
        embed = discord.Embed(
            title="<:MekoExclamation:1302243537924132875> No Prefix Expired",
