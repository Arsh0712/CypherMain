...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await self.user.send(f"<:MekoExclamation:1302243537924132875> You have been Banned from **{self.author.guild.name}** by **{self.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
...
class Unban(commands.Cog):
    ...
    async def unban(self, ctx, user: discord.User, *, reason=None):
        ...
        try:
            await user.send(f"<a:H_TICK:1302972336039006229> You have been unbanned from **{ctx.guild.name}** by **{ctx.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:MekoUser:1302242484427948172> Target User:** [{user}](https://discord.com/users/{user.id})\n**<:MekoPing:1302242859537272852> User Mention:** {user.mention}\n**<:Mail:1310249659595358239> DM Sent:** {dm_status}\n**<:MekoSearch:1302242746932789270> Reason:** {reasonn}", color=self.color)
