...
class AFKView(discord.ui.View):
    ...
    async def server_afk(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            description=f"<:MekoArrowRight:1302273350743097395> **Reason:** {self.reason}",
            color=0x00ff00
        )
...
class AFKCog(commands.Cog):
    ...
    async def afk(self, ctx, *, reason: str = "I am afk :)"):
        if any(word in reason.lower() for word in ['discord.gg', '.gg/', 'https', 'http', 'www', 'guns.lol']):
            emd = discord.Embed(description=f"<:icons_info:1354798914548666589> {ctx.author.mention}: You can't advertise in your AFK reason.", color=0x00FFCA)
        ...
        async with aiosqlite.connect('database/afk.db') as db:
            for user in message.mentions:
                ...
                if row:
                    ...
                    embed = discord.Embed(
                        description=f"<:MekoArrowRight:1302273350743097395> **Reason Was:** {reason}",
                        color=0xffff00
                    )
