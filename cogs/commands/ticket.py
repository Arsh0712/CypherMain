...
class TopicDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(
                f"<:ded:1323930463235997737> | {interaction.user.mention}: Your ticket has been created - {ticket_channel.mention}",
                ephemeral=True
            )
...
class Ticket(commands.GroupCog, name="ticket"):
    ...
    async def topic_create(self, ctx, topic_name: str, emoji: discord.Emoji = None):
        try:
            ...
            if topic_count >= 5:
                await ctx.send(embed=discord.Embed(description=f"<:ded:1323930463235997737> {ctx.author.mention}: You cannot create more than 5 topics per server.",color=0xFEE75C))
                return
            ...
            await ctx.send(embed=discord.Embed(
                description=f"<:ded:1323930463235997737> {ctx.author.mention}: Topic `{topic_name}` has been created.",
                color=0xFEE75C
            ))

        except sqlite3.IntegrityError:
            await ctx.send(embed=discord.Embed(
                description=f"<:danger:1323479256424120321> | Topic by the name `{topic_name}` already exists in this server.",
                color=0xFEE75C
            ))
    ...
    async def topic_rename(self, ctx, topic_name: str, new_name: str):
        try:
            ...
            if not topic:
                await ctx.send(embed=discord.Embed(
                    description=f"<:icons_info:1354798914548666589> {ctx.author.mention}: Topic `{topic_name}` does not exist.",
                    color=0xFEE75C
                ))
                return
            self.cursor.execute(
            ...
            await ctx.send(embed=discord.Embed(
                description=f"<:ded:1323930463235997737> {ctx.author.mention}: Topic `{topic_name}` has been renamed to `{new_name}`.",
                color=0x71368a
            ))

        except sqlite3.IntegrityError:
            await ctx.send(embed=discord.Embed(
                description=f"<:icons_info:1354798914548666589> {ctx.author.mention}: A topic named `{new_name}` already exists.",
                color=0xFEE75C
            ))
    ...
    async def topic_emoji_update(self, ctx, topic_name: str, new_emoji: discord.Emoji):
        ...
        try:
            ...
            await ctx.send(embed=discord.Embed(
                description=f"<:ded:1323930463235997737> {ctx.author.mention}: Emoji for topic `{topic_name}` has been updated to {str(new_emoji)}.",
                color=embed_color
            ))

        except Exception as e:
            await ctx.send(embed=discord.Embed(
                description=f"<:icons_info:1354798914548666589> {ctx.author.mention}: An error occurred: Please contact my staff team`",
                color=0xFEE75C
            ))
    ...
    async def topic_remove(self, ctx, *, topic_name: str):
        ...
        if self.cursor.rowcount:
            self.conn.commit()
            await ctx.send(embed=discord.Embed(description=f"<:ded:1323930463235997737> {ctx.author.mention}: Topic `{topic_name}` has been removed from the database.", color=embed_color))
        else:
            await ctx.send(embed=discord.Embed(description=f"<:icons_info:1354798914548666589> {ctx.author.mention}: Topic by the name `{topic_name}` does not exist in this server.", color=0xFEE75C))
    ...
    async def set_category(self, ctx, topic: str, category_id: str):
        ...
        if self.cursor.rowcount:
            self.conn.commit()
            await ctx.send(embed=discord.Embed(description=f"<:ded:1323930463235997737> {ctx.author.mention}: Category for topic `{topic}` has been set to `{category_id}`.", color=embed_color))
        else:
            await ctx.send(embed=discord.Embed(description=f"<:icons_info:1354798914548666589> {ctx.author.mention}: Topic by the name `{topic}` does not exist in this server.", color=0xFEE75C))
    ...
    async def set_support(self, ctx, topic: str, support_role: discord.Role):
        ...
        if self.cursor.rowcount:
            self.conn.commit()
            await ctx.send(embed=discord.Embed(description=f"<:ded:1323930463235997737> {ctx.author.mention}: Support for topic `{topic}` has been set to {support_role.mention}.", color=embed_color))
        else:
            await ctx.send(embed=discord.Embed(description=f"<:icons_info:1354798914548666589> {ctx.author.mention}: Topic by the name `{topic}` does not exist in this server.", color=0xFEE75C))
    ...
    async def setup_ticket(self, ctx, *, code: str = None):
        ...
        embed.set_footer(text=f"{self.bot.user.name} Ticket", icon_url=self.bot.user.avatar.url)
