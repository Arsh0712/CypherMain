...
class Automod(commands.Cog):
    ...
    async def enable(self, ctx):
        ...
        if ctx.author != ctx.guild.owner and ctx.author.top_role.position < ctx.guild.me.top_role.position:
            embed = discord.Embed(description="<a:MekoCross:1302972393572143125> Your top role must be at the **same** position or **higher** than my top role.", color=0x000000)
        ...
        if await self.is_automod_enabled(guild_id):
            embed=discord.Embed(description=f"**<:MekoExclamation:1302243537924132875> Your Server already has Automod Enabled.**\n\nCurrent Status: True\nTo Disable use `{ctx.prefix}automod disable`", color=0x000000)
    ...
    async def punishment(self, ctx):
        ...
        async def select_callback(interaction):
            if interaction.user != ctx.author:
                embed = discord.Embed()
                embed.description = f"<:MekoExclamation:1302243537924132875> I'm sorry, **{interaction.user.name}**, you cannot access this menu.\n<:MekoArrowRight:1302273350743097395> Please use the bot command first then you can access this menu"
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
    ...
    async def ignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        async with aiosqlite.connect("database/automod.db") as db:
            ...
            if await cursor.fetchone() is not None:
                embed = discord.Embed(description=f"<a:MekoCross:1302972393572143125> The channel {channel.mention} is already in the ignore list.\n\n<:MekoArrowRight:1302273350743097395> Use **{ctx.prefix}automod unignore channel {channel.mention}** to remove it.", color=0x000000)
            ...
            success = discord.Embed( description=f"The channel {channel.mention} has been added to the ignore list \n\n<:MekoArrowRight:1302273350743097395> Use `{ctx.prefix}automod ignore show` to view the ignore list.", color=0x000000)
    ...
    async def ignore_role(self, ctx, role: discord.Role):
        guild_id = ctx.guild.id
        if ctx.author != ctx.guild.owner and ctx.author.top_role.position < ctx.guild.me.top_role.position:
            embed = discord.Embed(title="<a:MekoCross:1302972393572143125> Access Denied", description="Your top role must be at the **same** position or **higher** than my top role.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
            return
        ...
        async with aiosqlite.connect("database/automod.db") as db:
            ...
            if await cursor.fetchone() is not None:
                embed = discord.Embed(description=f"<a:MekoCross:1302972393572143125> The role {role.mention} is already in the ignore list.\n\n<:MekoArrowRight:1302273350743097395> Use **{ctx.prefix}automod unignore role {role.mention}** to remove it.", color=0x000000)
                embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                await ctx.send(embed=embed)
                return
            ...
            success = discord.Embed(description=f"<a:H_TICK:1302972336039006229> The role {role.mention} has been added to the ignore list \n\n<:MekoArrowRight:1302273350743097395> Use `{ctx.prefix}automod ignore show` to view the ignore list.", color=0x000000)
            success.set_thumbnail(url=self.bot.user.avatar.url)
            success.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
    ...
    async def ignore_reset(self, ctx):
        ...
        async with aiosqlite.connect("database/automod.db") as db:
            ...
            await db.commit()
        embed=discord.Embed(description=f"** <a:H_TICK:1302972336039006229> All ignored channels and roles have been reset!**\n\nTo view current Automod settings use `{ctx.prefix}automod config`", color=0x000000)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
        embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
               icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=embed)
    ...
    async def unignore_role(self, ctx, role: discord.Role):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(description=f"<a:H_TICK:1302972336039006229> {role.mention} has been removed from the automod ignore list.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(description=f"<a:MekoCross:1302972393572143125> {role.mention} is not in the automod ignore list.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def logging(self, ctx, channel: discord.TextChannel):
        ...
        async with aiosqlite.connect("database/automod.db") as db:
            ...
            await db.commit()
            embed=discord.Embed(description=f"**<a:H_TICK:1302972336039006229> Automod Logging channel set to {channel.mention}.**\n\n<:MekoArrowRight:1302273350743097395> Use `{ctx.prefix}automod config` to view current Automod settings.", color=0x000000)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=embed)
