...
OWNER_IDS = {1263792569981341819, 1264585642780790814, 416875242108092426, 1365302949030527029, 807193356026904587}
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if log_channel:
            embed = discord.Embed(
                ...
                description=f"**<:MekoUser:1302242484427948172> User**: [{self.user}](https://discord.com/users/{self.user.id})\n**<:MekoPing:1302242859537272852> User Mention**: {self.user.mention}\n**<:MekoBan:1302243547780743199> Added By**: [{self.author.display_name}](https://discord.com/users/{self.author.id})\n<:MekoTimer:1302243147547541514> **Expiry Time**: {expiry_text}\n<:MekoSearch:1302242746932789270> **Timestamp**: {expiry_timestamp}\n\n<:MekoRuby:1303665457965826089> **Tier**: **{self.values[0].upper()}**",
                color=0x000000
            )
        ...
        embed = discord.Embed(description=f"**Added Global No Prefix**:\n<:MekoUser:1302242484427948172> **User**: **[{self.user}](https://discord.com/users/{self.user.id})**\n<:MekoPing:1302242859537272852> **User Mention**: {self.user.mention}\n<:MekoBan:1302243547780743199> **Added By**: **[{self.author.display_name}](https://discord.com/users/{self.author.id})**\n<:MekoTimer:1302243147547541514> **Expiry Time:** {expiry_text}\n<:MekoSearch:1302242746932789270> **Timestamp:** {expiry_timestamp}", color=0x000000)
...
class Noprefix(commands.Cog):
                        ...
                        if user:
                            ...
                            if log_channel:
                                embed_log = discord.Embed(
                                    ...
                                    description=(
                                        f"**<:MekoUser:1302242484427948172> User**: [{user}](https://discord.com/users/{user.id})\n"
                                        f"**<:MekoPing:1302242859537272852> User Mention**: {user.mention}\n"
                                        f"**<:MekoBan:1302243547780743199> Removed By**: **[Cypher#2733](https://discord.com/users/1191399940014997584)**\n"
                                        ),
                                    color=0x000000
                                    )
                                ...
                                if guild:
                                    ...
                                    if member:
                                        ...
                                        if role in member.roles:
                                            ...
                                            embed = discord.Embed(
                                                description=f"<:MekoExclamation:1302243537924132875> Your No Prefix status has been **Expired**. You will now require the prefix to use commands.\n> Example : **.help**",
                                                color=0x000000
                                                )
                                            ...
                                            support = Button(label='Support',
                                                             ...
                                                             url=f'https://discord.gg/XBbHRYg4ZW')
                                            view = View()
    ...
    async def np_add(self, ctx, user: discord.User):
        async with self.db.execute("SELECT id FROM np WHERE id = ?", (user.id,)) as cursor:
            ...
            if result:
                embed = discord.Embed(description=f"**{user}** is already in No prefix list\n\n<:MekoBan:1302243547780743199> **Requested By**: [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})\n", color=0x000000)
    ...
    async def np_remove(self, ctx, user: discord.User):
        async with self.db.execute("SELECT id FROM np WHERE id = ?", (user.id,)) as cursor:
            result = await cursor.fetchone()
            if not result:
                embed = discord.Embed(description=f"**{user}** is not in the No Prefix list\n\n<:MekoBan:1302243547780743199> **Removed By**: [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})\n", color=0x000000)
                await ctx.reply(embed=embed)
                return
                    ...
                    if role in member.roles:
                        ...
                        embed = discord.Embed(
                            description=(
                                f"**<:MekoUser:1302242484427948172> User**: [{user}](https://discord.com/users/{user.id})\n"
                                f"**<:MekoPing:1302242859537272852> User Mention**: {user.mention}\n"
                                f"**<:MekoBan:1302243547780743199> Removed By**: [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})\n"
                                ),
                            color=0x000000
                            )
        ...
        async with self.db.execute("SELECT id, expiry_time FROM np WHERE id = ?", (user.id,)) as cursor:
            ...
            if not result:
                embed = discord.Embed(
                    ...
                    f"<:MekoMember:1302243182804860938> **Requested By**: "
            ...
            if expires and expires != "Null": 
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        f"**<:MekoUser:1302242484427948172> User**: [{user}](https://discord.com/users/{user.id})\n"
                        f"**<:MekoTimer:1302243147547541514> Expiry**: {expire_time} ({expire_timestamp})"
                        ),
                    color=0x000000
                    )
