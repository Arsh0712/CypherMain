...
class TimeoutView(ui.View):
    ...
    @ui.button(style=discord.ButtonStyle.gray, emoji="<:MekoTrash:1302929643514499114>")
...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(description=f"**<:MekoUser:1302242484427948172> Target User:** [{self.user}](https://discord.com/users/{self.user.id})\n<:MekoPing:1302242859537272852> **User Mention:** {self.user.mention}\n**<:Mail:1310249659595358239> DM Sent:** {dm_status}\n**<:MekoSearch:1302242746932789270> Reason:** {reason}", color=0x000000)
...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, user: discord.Member, time: str = None, *, reason=None):
        ...
        if user == ctx.guild.owner:
            error = discord.Embed(color=self.color, description="<:MekoExclamation:1302243537924132875> You can't timeout the Server Owner!")
        ...
        if ctx.author != ctx.guild.owner and user.top_role >= ctx.author.top_role:
            error = discord.Embed(color=self.color, description="<:MekoExclamation:1302243537924132875> You can't timeout users having higher or equal role than yours!")
            error.set_footer(text=f"Requested by {ctx.author}", icon_url=self.get_user_avatar(ctx.author))
            return await ctx.send(embed=error)

        if user.top_role >= ctx.guild.me.top_role:
            error = discord.Embed(color=self.color, description="<:MekoExclamation:1302243537924132875> I can't timeout users having higher or equal role than mine.")
            error.set_footer(text=f"Requested by {ctx.author}", icon_url=self.get_user_avatar(ctx.author))
            return await ctx.send(embed=error)
        ...
        try:
            await user.send(f"<:MekoExclamation:1302243537924132875> You have been muted in **{ctx.guild.name}** by **{ctx.author}** for {duration_text}. Reason: {reason or 'None'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:MekoUser:1302242484427948172> Target User:** [{user}](https://discord.com/users/{user.id})\n"
                                          f"<:MekoPing:1302242859537272852> **User Mention:** {user.mention}\n"
                                          f"<:Mail:1310249659595358239> **DM Sent:** {dm_status}\n"
                                          f"**<:MekoSearch:1302242746932789270> Reason:** {reason or 'None'}\n"
                                          f"**<:MekoTimer:1302243147547541514> Duration:** {duration_text}",
                              color=self.color)
    ...
    async def mute_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            embed = discord.Embed(title="<a:MekoCross:1302972393572143125> Operation Failed", description="I don't have permission to mute members.", color=self.color)
            await ctx.send(embed=embed)
        elif isinstance(error, discord.Forbidden):
            embed = discord.Embed(title="<a:MekoCross:1302972393572143125> Missing Permissions", description="I can't mute this user as they might have higher privileges (e.g., Admin).", color=self.color)
            await ctx.send(embed=embed)
