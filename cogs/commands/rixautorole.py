...
class AutoRoleManager:
    ...
    def add_role(self, guild_id: str, role_type: str, role_id: int) -> None:
        ...
        if at_limit:
            raise RoleLimit(f"<:MekoExclamation:1302243537924132875> Maximum limit of {max_roles} roles for {role_type} has been reached.")
...
class AutoRole(commands.Cog):
    ...
    async def autorole(self, ctx):
        ...
        if view.value is None:
            error_embed = discord.Embed(description="<:MekoWarn:1302265988666101862> **Autorole Setup Menu** has been timed out.", color=discord.Color.red())
...
class AutoroleView(discord.ui.View):
    ...
    async def autorole_add(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.user_id:
            embed = discord.Embed(
                description=(
                    f"<:MekoExclamation:1302243537924132875> I'm sorry, **{interaction.user.name}**, you cannot access this button.\n"
                    f"<:MekoArrowRight:1302273350743097395> Please use the bot command first then you can access this button."
                )
            )
        ...
        embed = discord.Embed(
            description="<:MekoArrowRight:1302273350743097395> Please choose an option from the provided buttons below to setup an autorole for `Humans` or `Bots`."
        )
        embed.set_author(name=f"Autorole Menu", icon_url=self.bot.user.avatar)
...
class AutoroleSubmenuView(discord.ui.View):
    ...
    async def setup_roles(self, interaction: discord.Interaction, role_type: str):
        ...
        embed = discord.Embed(
            description="<:MekoArrowRight:1302273350743097395> Please select one or more roles from the selection menu to assign as human autoroles.\n\n<:MekoRuby:1303665457965826089> **Note :** If your server roles have custom font names you may not be able to find them in the selection menu, so in that case use the `Add Roles` button."
        )
        embed.set_author(name=f"Autorole Menu", icon_url=self.bot.user.avatar)
...
class UseMentionButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed= discord.Embed()
        embed.description="<:MekoArrowRight:1302273350743097395> Please send upto 5 role Mentions or IDs in this channel."
    ...
    async def back(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            description=(
                "<:MekoArrowRight:1302273350743097395> Please choose an option from the provided buttons below.\n\n"
