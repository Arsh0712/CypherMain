...
class SnipeView(discord.ui.View):
    ...
    async def send_snipe_embed(self, interaction: discord.Interaction):
        ...
        embed.description = (
            f"<:MekoUser:1302242484427948172> **Author:** **[{display_name}](https://discord.com/users/{uid})**\n"
            f"<:id:1303722827748343972> **Author ID:** `{snipe['author_id']}`\n"
            f"<:MekoPing:1302242859537272852> **Author Mention:** <@{snipe['author_id']}>\n"
            f"<:MekoTimer:1302243147547541514> **Deleted:** <t:{snipe['deleted_at']}:R>\n"
        )

        if snipe['content']:
            embed.add_field(name="<:MekoTrash:1302929643514499114> **Content:**", value=snipe['content'])
        if snipe['attachments']:
            attachment_links = "\n".join([f"[{attachment['name']}]({attachment['url']})" for attachment in snipe['attachments']])
            embed.add_field(name="<:MekoAttachment:1302243192779051079> **Attachments:**", value=attachment_links)
    ...
    @discord.ui.button(emoji="<:MekoDoubleArrowLeft:1302274910856413316>", style=discord.ButtonStyle.secondary, custom_id="first")
    async def first_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.index = 0
        self.update_buttons()
        await self.send_snipe_embed(interaction)

    @discord.ui.button(emoji="<:MekoArrowLeft:1302273353507405854>", style=discord.ButtonStyle.secondary, custom_id="previous")
    ...
    @discord.ui.button(emoji="<a:MekoCross:1302972393572143125>", style=discord.ButtonStyle.danger, custom_id="delete")
    async def delete_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.message.delete()

    @discord.ui.button(emoji="<:MekoArrowRight:1302273350743097395>", style=discord.ButtonStyle.secondary, custom_id="next")
    ...
    @discord.ui.button(emoji="<:MekoDoubleArrowRight:1302274914325106730>", style=discord.ButtonStyle.secondary, custom_id="last")
...
class Snipe(commands.Cog):
    ...
    async def snipe(self, ctx):
        ...
        embed.description = (
            f"<:MekoUser:1302242484427948172> **Author:** **[{display_name}](https://discord.com/users/{uid})**\n"
            f"<:id:1303722827748343972> **Author ID:** `{first_snipe['author_id']}`\n"
            f"<:MekoPing:1302242859537272852> **Author Mention:** <@{first_snipe['author_id']}>\n"
            f"<:MekoTimer:1302243147547541514> **Deleted:** <t:{first_snipe['deleted_at']}:R>\n"
        )

        if first_snipe['content']:
            embed.add_field(name="<:MekoTrash:1302929643514499114> **Content:**", value=first_snipe['content'])
