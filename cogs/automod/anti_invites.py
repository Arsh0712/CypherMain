    ...
    async def on_message(self, message):
        ...
        if self.invite_pattern.search(message.content):
            ...
            try:
                ...
                try:
                    if punishment == "Mute":
                        timeout_duration = discord.utils.utcnow() + timedelta(minutes=10)
                        await user.edit(timed_out_until=timeout_duration, reason="Posted an invite link")
                        action_taken = "`10` minutes ."
                    ...
                    simple_embed.description = f"<:MekoAutomod:1302243158851059775> You've been timed out by **Cypher Automod** for {action_taken}\n<:MekoRuby:1303665457965826089> **Reason :** posting an invite link."
...
async def setup(client):
    await client.add_cog(AntiInvite(client))